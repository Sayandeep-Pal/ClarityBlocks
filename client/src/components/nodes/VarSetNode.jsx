// client/src/components/nodes/VarSetNode.js
import React from 'react';
import { Handle, Position } from 'react-flow-renderer';
import { RiDatabaseLine } from 'react-icons/ri';
import { FaCog, FaTrash } from 'react-icons/fa';

const VarSetNode = ({ id, data, isConnectable, onUpdateNodeData, onDeleteNode }) => {
    ...
    <div className="bg-blueGrayBlock border border-gray-600 rounded-lg shadow-xl w-64 overflow-hidden relative">
      <div className="flex items-center justify-between p-3 bg-gray-700 text-white font-bold text-lg">
        <div className="flex items-center">
          <RiDatabaseLine className="mr-2 text-xl text-red-500" />
          Var Set
        </div>
      ...
      <div className="p-3 text-gray-100 text-sm">
        ...
        <div className="mb-2">
          <label className="font-semibold text-gray-200 block mb-1">Value (e.g., (+ (var-get my-data-var) u1)):</label>
          <input
            type="text"
            name="value"
            value={data.value || ''}
      ...
      <Handle type="target" position={Position.Left} id="in" isConnectable={isConnectable} className="w- 8 h-8 rounded-full bg-gray-300 border-2 border-gray-700" />
      <Handle type="source" position={Position.Right} id="out" isConnectable={isConnectable} className="w- 8 h-8 rounded-full bg-gray-300 border-2 border-gray-700" />
    </div>
  );
};

export default VarSetNode;