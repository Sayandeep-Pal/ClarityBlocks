...
const ConstantNode = ({ id, data, isConnectable, onUpdateNodeData, onDeleteNode }) => {
  ...
  return (
    <div className="bg-yellowBlock border border-yellow-600 rounded-lg shadow-xl w-64 overflow-hidden relative">
      ...
      <Handle type="target" position={Position.Left} id="in" isConnectable={isConnectable} className="w- 8 h-8 rounded-full bg-yellow-300 border-2 border-yellow-700" />
      <Handle type="source" position={Position.Right} id="out" isConnectable={isConnectable} className="w- 8 h-8 rounded-full bg-yellow-300 border-2 border-yellow-700" />
    </div>
  );
};
