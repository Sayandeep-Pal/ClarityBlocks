...
const ReadOnlyFunctionNode = ({ id, data, isConnectable, onUpdateNodeData, onDeleteNode }) => {
  ...
  return (
    <div className="bg-cyanBlock border border-cyan-600 rounded-lg shadow-xl w-64 overflow-hidden relative">
      ...
      <Handle type="target" position={Position.Left} id="in" isConnectable={isConnectable} className="w-18 h-18 rounded-full bg-cyan-300 border-2 border-cyan-700" />
      <Handle type="source" position={Position.Right} id="out" isConnectable={isConnectable} className="w-18 h-18 rounded-full bg-cyan-300 border-2 border-cyan-700" />
    </div>
  );
};
